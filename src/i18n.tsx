...
const ID: Dict = {
  ...
  'inc.rekap.rekonSd': ' · s/d {bulan}',
  'inc.rekap.hint':
    'Tunai + QRIS diambil dari metode pembayaran tiap laporan; gaji dihitung dari slip semua karyawan bulan ini; lain-lain dari halaman Pengeluaran. "Cek saldo" dihitung KUMULATIF s/d bulan terpilih memakai Saldo Awal, sumber dana tiap pengeluaran, & gaji yang sudah dibayar (Tunai → dompet, selain itu → rekening): dompet = saldo awal + Σ tunai − Σ setoran − Σ pengeluaran(cash) − Σ gaji dibayar(tunai); rekening = saldo awal + Σ QRIS + Σ setoran − Σ pengeluaran(rekening) − Σ gaji dibayar(transfer).',
  ...
  'inc.setor.total': 'Setoran tunai → rekening',
  'inc.setor.totalSd': 'Total setoran s/d bulan ini',
  'inc.setor.dompetHarap': 'Seharusnya di dompet: {rp}',
  'inc.setor.rekeningHarap': 'Seharusnya di rekening: {rp}',
...
const EN: Dict = {
  ...
  'inc.rekap.rekonSd': ' · through {bulan}',
  'inc.rekap.hint':
    'Cash + QRIS come from each report’s payment method; salary is computed from all employees’ slips this month; other expenses from the Expenses page. "Balance check" is CUMULATIVE through the selected month using the Opening Balance, each expense’s fund source, & salaries already paid (Cash → wallet, otherwise → account): wallet = opening + Σ cash − Σ deposits − Σ expenses(cash) − Σ salary paid(cash); account = opening + Σ QRIS + Σ deposits − Σ expenses(account) − Σ salary paid(transfer).',
  ...
  'inc.setor.total': 'Cash deposited → account',
  'inc.setor.totalSd': 'Total deposits through this month',
  'inc.setor.dompetHarap': 'Expected in wallet: {rp}',
  'inc.setor.rekeningHarap': 'Expected in account: {rp}',
